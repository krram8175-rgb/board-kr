...
const PART_NAME = { A: "1 Mark", B: "2 Marks", C: "3 Marks", D: "5 Marks", E: "6 Marks", E4: "4 Marks" };
...
export default function ChapterDetail() {
  ...
  const parts = bp && row
    ? bp.parts
        .filter((p) => row.vals[p.key])
        .map((p) => ({
          key: p.key,
          mark: p.mark,
          name: PART_NAME[p.key] || `${p.mark} Marks`,
          count: row.vals[p.key],
          marks: row.vals[p.key] * Number(p.mark),
        }))
    : [];
  const totalQ = parts.reduce((s, p) => s + p.count, 0);
            ...
            <div className="mb-6 flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {row.marks} marks weightage · {totalQ} question{totalQ === 1 ? "" : "s"} · {row.hours} teaching hours
                </p>
              </div>
            </div>

            <h2 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-500">Weightage by question type</h2>
              ...
              {parts.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-extrabold text-slate-900">{p.name} Question{p.count === 1 ? "" : "s"}</p>
                      <p className="text-xs text-slate-500">{p.count} × {p.mark} mark</p>
                    </div>
